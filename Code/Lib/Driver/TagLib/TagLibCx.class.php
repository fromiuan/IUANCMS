...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'field'     =>  array('attr'=>'name,fromvo,prefix,function','close'=>0),
        'current'   =>  array('attr'=>'field,type,value','level'=>2),
        );
    ...
    public function _field($attr){
        ...
        //变量前缀
        $var_prefix = empty($fromvo) ? '$_field': '$vo_'.$fromvo;
        $var_prefix = empty($prefix) ? $var_prefix : '$'.$prefix;
     ...
     * current标签解析  
     * 格式：
     * <current field='typeid' type='eq' value='5'>
     * <else/>
     * </current>
     * @access public
     * @param string $attr 标签属性
     ...
     * @return string
     */
    public function _current($attr,$content) {
        $tag        =   $this->parseXmlAttr($attr,'current');
        ...
        switch($a)
        {
            ...
            break;
            case ':':
            $key = ltrim($field,':');//支持函数调用
            break;
            case '~':
            $field = trim($field,'~');
            $key = '$GLOBALS[\'_fields\'][\''.$field.'\']';//支持处理页面全局变量 
            break;
            default:
            $key = '$_field[\''.$field.'\']';
        }       
        if($value==null && !in_array($a,array('~',':','$')))
        {   
            $value = '$GLOBALS[\'_fields\'][\''.$field.'\']';//为空则从全局变量获取
            $noglobal = 0;//从全局变量获取的标识
        }
        $parseStr ='<?php '; 
        switch($type)
        {
            case 'eq':
            $parseStr.= $noglobal == 0 ? " if({$key} == {$value}):?>" : " if({$key} == '{$value}'):?>";
            break;
            case 'gt':
            $parseStr.= $noglobal == 0 ? " if({$key} > {$value}):?>" : " if({$key} > '{$value}'):?>";
            break;
            ...
            break;
            case 'empty':
            if($a <> ':') $parseStr.= " if(empty({$key})):?>";
            break;
            case 'notempty':
            if($a <> ':') $parseStr.= " if(!empty({$key})):?>";
            break;
            case 'present':
            if($a <> ':') $parseStr.= " if(isset({$key})):?>";
            break;
            case 'notpresent':
            if($a <> ':') $parseStr.= " if(!isset({$key})):?>";
            break;
            case 'in':
            $parseStr.= '$__ARRAY__ = array('.$value.');';
            $parseStr.= ' if(in_array('.$key.',$__ARRAY__)):?>';
            break;
            case 'notin':
            $parseStr.= '$__ARRAY__ = array('.$value.');';
            $parseStr.= ' if(!in_array('.$key.',$__ARRAY__)):?>';
            ...
            case 'between':
            $parseStr.= '$__ARRAY__ = array('.$value.');';
            $parseStr.= ' if('.$key.' >$__ARRAY__[0] && '.$key.' < $__ARRAY__[1]):?>';
            break;
            case 'notbetween':
            $parseStr.= '$__ARRAY__ = array('.$value.');';
            $parseStr.= ' if('.$key.' < $__ARRAY__[0] && '.$key.' > $__ARRAY__[1]):?>';
            ...
            if($a=='~')
            {
                $condition = strtr($value,array('@me'=>'$GLOBALS[\'_fields\'][\''.$field.'\']','gt'=>'>','lt'=>'<','egt'=>'>=','elt'=>'<='));
            }
            elseif($a=='$')
            {
                $condition = strtr($value,array('@me'=>$field,'gt'=>'>','lt'=>'<','egt'=>'>=','elt'=>'<='));
            }
            else
            {
                $condition = strtr($value,array('@me'=>'$_field[\''.$field.'\']','gt'=>'>','lt'=>'<','egt'=>'>=','elt'=>'<='));
            }
            $parseStr.= ' if('.$condition.'):?>';
            break;
        }
