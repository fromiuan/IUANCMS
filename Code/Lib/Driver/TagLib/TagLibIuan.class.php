...
/***********************************************************
  ...
  @function 核心框架 标签解析
    ...
    @Filename TaglibIuan.class.php $

    @Author liucan $

    @Date 2013-11-1 12:10:11 $
*************************************************************/
defined('THINK_PATH') or exit();
class TagLibIuan extends TagLib {
    // 标签定义
    ...
    'global'   =>  array('attr'=>'name','close'=>0),
    'tplvar'   =>  array('attr'=>'name,function','close'=>0),
    'field'    =>  array('attr'=>'name,function','close'=>0),
    'arctype'  =>  array('attr'=>'diymodel,vo,fromvo,typeid,row,limit,field,type,modeltype,modelid,key,mod,order','level'=>3),
   ...
   public function _arclist($attr,$content)
   {
    ...
    $orderby    =    isset($tag['orderby'])  ? $tag['orderby'] : 'createtime desc';
    ...
    $field .= "concat('{$root}/show_', blog.id, '{$suffix}') as arcurl,";
    $field .= "concat('<a href=\\\"{$root}/show_', blog.id, '{$suffix}\\\">',title,'</a>') as arclink,";
    $field .= "concat('{$root}/list_', blog.typeid, '{$suffix}') as typeurl,";
    $field .= "concat('<a href=\\\"{$root}/list_', blog.typeid, '{$suffix}\\\">',name,'</a>') as typelink,";
    $field .= 'id,createtime,litpic,name,title as fulltitle,';
    empty($titlelen) ?  $field.='title' : $field.= "left(blog.title,{$titlelen}) as title";
    if(empty($map['_string'])){ unset($map['_string']);}else{ $map['_string'] = trim($map['_string'],'AND');}
    //record判断
    if($record =='~createtime~' && !empty($_fields['createtime']) && empty($subday)) $map['createtime'] = array('egt',$_fields['createtime']);
    $sql = $model->field($field)->where($map)->order($orderby)->limit($limit)->select(false);
    //循环标识判断
    $fromvotag = empty($fromvo) ? '$_field' : '$vo_'.$fromvo;
    if($typeid=='~typeid~' && $getall <> 1) $sql = str_replace(') ORDER BY','AND blog.typeid=\'".'.$fromvotag.'[\'typeid\']."\') ORDER BY',$sql);
    if($typeid=='~typeid~' && $getall == 1) $sql = str_replace(') ORDER BY','AND blog.typeid IN ("._arclist_getall_sonid('.$fromvotag.'[\'typeid\']).") ) ORDER BY',$sql);
    $parseStr   =  '<?php ';
      //分页支持
    if($paging==1)
    { 
      //获取全局变量时动态统计sql获取记录总数
      $sql2 = $model->field("count(*) as wk_num")->where($map)->order('createtime desc')->select(false);
      if($_getglobal==1 && $getall <> 1) 
      {
        $sql2 = str_replace(') ORDER BY','AND blog.typeid=\'".$GLOBALS[\'_fields\'][\'typeid\']."\') ORDER BY',$sql2);
        $parseStr  .='$totalnum = query("'.$sql2.'");';
        $parseStr  .='$totalnum = $totalnum[0][\'wk_num\'];';
      }
      elseif($_getglobal==1 && $getall == 1)
      {
        $sql2 = str_replace(') ORDER BY','AND blog.typeid IN ("._arclist_getall_sonid($GLOBALS[\'_fields\'][\'typeid\']).") ) ORDER BY',$sql2);
        $parseStr  .='$totalnum = query("'.$sql2.'");';
        $parseStr  .='$totalnum = $totalnum[0][\'wk_num\'];';
      }
      else
    ...
    //获取全局变量时动态生成sql语句
    if($_getglobal==1 && $getall <> 1) {$sql = str_replace(') ORDER BY','AND blog.typeid=\'".$GLOBALS[\'_fields\'][\'typeid\']."\') ORDER BY',$sql);}
    if($_getglobal==1 && $getall == 1) {$sql = str_replace(') ORDER BY','AND blog.typeid IN ("._arclist_getall_sonid($GLOBALS[\'_fields\'][\'typeid\']).") ) ORDER BY',$sql);}
    //自定义天数则 动态设置sql
    if(!empty($subday)) 
    {
    ...
    if($orderby =='~orderby~')
    {
      $parseStr.= '$__ORDERBY__ = empty($_REQUEST[\'orderby\']) ? \'createtime desc\':$_REQUEST[\'orderby\'];';
   ...
   public function _arctype($attr,$content)
   {
    ...
    $_getglobal = 0;
    //查询数据库
    $model = empty($diymodel) ? M('cate'):D($diymodel);
    if(!empty($modelid)) $map['modelid'] = array('in',$modelid);
    //获取全局变量typeid
    ...
    $suffix = '.'.C('URL_HTML_SUFFIX');
    if(!empty($field)) $field = rtrim($field,',').',';
    $field .= "concat('{$root}/list_', id, '{$suffix}') as typeurl,";
    $field .= "concat('<a href=\\\"{$root}/list_', id, '{$suffix}\\\">',name,'</a>') as typelink,";
    $field .= 'id as typeid,name';
    if(empty($limit)) $limit = '0,'.$row;
    $sql = $model->field($field)->where($map)->order($order)->limit($limit)->select(false);
    //循环标识判断
    $fromvotag = empty($fromvo) ? '$_field' : '$vo_'.$fromvo;
   ...
   public function _votelist($attr,$content)
  {
    ...
    $root  = $GLOBALS['cfg_rewrite']==0 ? __ROOT__ :__ROOT__.'/index.php';
    $suffix = '.'.C('URL_HTML_SUFFIX');
    $field .= "concat('{$root}/vote_', id, '{$suffix}') as voteurl,";
    $field .= "concat('<a href=\\\"{$root}/vote_', id, '{$suffix}\\\">',title,'</a>') as votelink,";
  ...
  public function _recordlist($attr,$content)
  {
    ...
    $model = empty($diymodel) ? M('blog'):D($diymodel);
