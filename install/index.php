...
define("VERSION", '2014-03-06 by IUAN');
$sqlFile = 'blog.sql';
$sqlFileDemo = 'blog_demo.sql';
$configFile = 'config.php';
if (!file_exists(SITEDIR . 'install/' . $sqlFile)) {
    echo '缺少数据库文件!';
    exit;
}
$Title = "IUANCMS PHP爱好者";
$Powered = "Powered by W3CDREAM";
$steps = array(
    '1' => '安装许可协议',
    '2' => '运行环境检测',
    '3' => '安装参数设置',
...
switch ($step) {
    ...
    case '2':
        ...
        $folder = array('/', 'Data', 'Data/Config/config.ini.php', 'Public/templets', 'install',);
    ...
    case '4':
        if (intval($_GET['install'])) {
            ...
            $verify = randCode(6);
            $config = array();
            $config['TMPL_FILE_DEPR']='_';

            $config['WEB_ROOT'] = trim($_POST['siteurl']); //网站域名
            $config['RBAC_SUPERADMIN'] = $email;
            ...
            $time = time();
            $ip = get_client_ip();
            $query = "INSERT INTO `{$dbPrefix}user` VALUES ('1', '{$email}','{$password}','{$time}','$ip','{$time}','0','','0');";
            mysql_query($query);
            $message = '成功添加管理员<br />成功写入配置文件<br>安装完成．';

            file_put_contents(SITEDIR . "/Data/Config/config.ini.php", ("<?php\treturn " . var_export($config, true) . ";?>"));
...
function sql_split($sql, $tablepre) {
    ...
    if ($tablepre != "hd_")
        $sql = str_replace("hd_", $tablepre, $sql);
