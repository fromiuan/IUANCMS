...
// +----------------------------------------------------------------------
	...
	*/
	...
	//	iuan 	@TplAction 
	//++++++++++++++++++++++++++++++++++++++++++++++++++
	function iuan_replace($str){
		$newurl = __ROOT__;
		$url = str_replace("./", "$newurl/", $str);
		return $url;
	}

	function iuan_replace_down($str){
		$newurl = "%2F";
		$url = str_replace("./", "$newurl", $str);
		$urlall = str_replace("/", "$newurl", $url);
		return $urlall;

	}

//*************博文属性 start*************//
	function iuan_modify_blog($str){
		switch ($str) {
			case '1':
				return f;
				break;
	...
	function iuan_blog_index($str){
		$data = explode(',', $str);
		foreach ($data as $key => $value) {
			switch ($value) {
				case 'f':
					$strs .="<strong style='color:#FF3300'>幻灯</strong>,";
					break;
				case 'c':
					$strs .="<strong style='color:#009999'>推荐</strong>,";
					break;
				case 'h':
					$strs .="<strong style='color:#FF0000'>热门</strong>,";
					break;
				case 'b':
					$strs .="<strong style='color:#000000'>加粗</strong>,";
					break;	
			}
		}
		$strs = rtrim($strs,',');
